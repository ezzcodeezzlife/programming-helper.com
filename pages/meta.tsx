...
export const Bottomtext = () => {
  return (
    <div className="bottom-text">
      <h2>Meta Tags from Description</h2>
      {/* Write three sentences about this tool  */}
      <p>Meta Tags from Description. Its easy and fast</p>
    </div>
  )
}
...
export default function translate() {
  ...
  const fetchData = async () => {
    const res = await fetch("/api/examples/meta", {
      method: "POST",
      headers: {
  ...
  if (!session) {
    return (
      <Layout>
        <Head>
          <title>Meta Tags from Description</title>
        ...
        <h1>Meta Tags from Description:</h1>

        <p>
          <textarea
            value={textup}
            placeholder="A site about cat images"
            onKeyDown={(e) => {
              if (e.key === "Tab") {
                e.preventDefault()
                // add tab to content
          ...
          <button onClick={buttonPressLogin}>
            Sign in to Get Meta Tags Description
          </button>
          {requestloading ? <p>Loading...</p> : <></>}

          <textarea
            placeholder={`<meta name="description" content="A site about cat images">
